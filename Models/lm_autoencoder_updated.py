...
parser.add_argument("result_file", help="Where to write the result file")
args = parser.parse_args()

# logging.basicConfig(filename = "Results/noisy-test.csv", format="%(message)s", \
logging.basicConfig(filename = "Results/" + args.result_file + ".csv", format="%(message)s", \
                    level=logging.INFO)
...
test_idx_list = random.sample(population = [i for i in range(len(testing_pairs)) if i % 2 == 0], k = NUM_TESTING)
test_idx_list.extend([i+1 for i in test_idx_list])
test_idx_list.sort()
testing_pairs = [testing_pairs[i] for i in test_idx_list]
